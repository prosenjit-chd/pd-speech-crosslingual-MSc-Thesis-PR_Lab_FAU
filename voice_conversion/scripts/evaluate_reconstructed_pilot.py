...
"""
evaluate_reconstructed_pilot.py

Purpose:
Stage 2 evaluation script.
1. Generates metadata indices for both original and reconstructed pilot sets:
   - C:\pd-speech-crosslingual\voice_conversion\metadata_generated_pilot\dataset_index_original_pilot.csv
   - C:\pd-speech-crosslingual\voice_conversion\metadata_generated_pilot\dataset_index_generated_pilot.csv
2. Reuses baseline FeatureExtractor to extract WavLM, Wav2Vec2, and XLSR features for layers [0, 4, 8, 11]
   ...
   - C:\pd-speech-crosslingual\voice_conversion\features_original_pilot\
   - C:\pd-speech-crosslingual\voice_conversion\features_generated_pilot\
3. Runs baseline classification scenarios (UAR, Accuracy, Sensitivity, Specificity, AUC) on both sets
   ...
   - C:\pd-speech-crosslingual\voice_conversion\outputs_original_pilot\
   - C:\pd-speech-crosslingual\voice_conversion\outputs_generated_pilot\
4. Generates a comparative analysis CSV and markdown log comparing the original pilot vs. generated pilot:
   - C:\pd-speech-crosslingual\voice_conversion\logs\pilot_evaluation_comparison_summary.csv
   - C:\pd-speech-crosslingual\voice_conversion\logs\hifigan_stage2_evaluation_log.md
"""
...
# Directory Constants
METADATA_DIR = PROJECT_ROOT / "voice_conversion" / "metadata_generated_pilot"
FEATURES_ORIG_DIR = PROJECT_ROOT / "voice_conversion" / "features_original_pilot"
FEATURES_GEN_DIR = PROJECT_ROOT / "voice_conversion" / "features_generated_pilot"
OUTPUTS_ORIG_DIR = PROJECT_ROOT / "voice_conversion" / "outputs_original_pilot"
OUTPUTS_GEN_DIR = PROJECT_ROOT / "voice_conversion" / "outputs_generated_pilot"
LOG_DIR = PROJECT_ROOT / "voice_conversion" / "logs"

SELECTION_LOG_PATH = LOG_DIR / "pilot_selection_log.csv"
COMPARISON_SUMMARY_PATH = LOG_DIR / "pilot_evaluation_comparison_summary.csv"
STAGE2_LOG_PATH = LOG_DIR / "hifigan_stage2_evaluation_log.md"

def create_dataset_indexes():
    print("=" * 60)
    print("Step 1: Generating Dataset Indexes for Original vs. Generated Pilot")
    print("=" * 60)
    
    if not SELECTION_LOG_PATH.exists():
        print(f"ERROR: Selection log not found at {SELECTION_LOG_PATH}. Run run_pilot_stage.py first.")
        sys.exit(1)
    ...
    with open(SELECTION_LOG_PATH, mode="r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            orig_name = Path(orig_path).name # e.g. AVPEPUDEA0001_readtext.wav
            speaker_id = orig_name.split("_")[0] if "_" in orig_name else orig_name.split(".")[0]
            
            # Original Preprocessed Pilot Entry (resampled & normalized input WAV)
            orig_file_path = f"voice_conversion/input_pilot_22050/{new_filename}"
            orig_entries.append({
            ...
            # Generated HiFi-GAN Pilot Entry (output wav)
            gen_file_path = f"voice_conversion/generated/{new_filename.replace('.wav', '_generated.wav')}"
            gen_entries.append({
    ...
    # Write Original Pilot Index
    orig_idx_path = METADATA_DIR / "dataset_index_original_pilot.csv"
    ...
    # Write Generated Pilot Index
    gen_idx_path = METADATA_DIR / "dataset_index_generated_pilot.csv"
...
def classify_features_for_set(features_dir, outputs_dir, config):
    ...
    # Small size adjustments for folds
    outer_folds = 3
    inner_folds = 2
...
def generate_comparisons_and_markdown(orig_results, gen_results):
    ...
    md_lines = []
    md_lines.append("# HiFi-GAN Stage 2: Feature & Classification Evaluation Report\n")
    md_lines.append("## Comparative Evaluation of Reconstructed vs. Original Pilot Audio\n")
    md_lines.append(f"This report compares the classification UAR and Accuracy scores for **original** vs. **HiFi-GAN reconstructed** audios across XLSR, Wav2Vec2, and WavLM models.\n")
    md_lines.append("> [!WARNING]")
    md_lines.append("> Due to the small size of this pilot (12 files total), cross-validation settings were restricted (3-fold outer CV, 2-fold inner CV). Performance scores represent diagnostic stability check values and representation drift rather than generalizable accuracy.\n")
    ...
    md_lines.append("- **Acoustic Diagnostic Preservation**: If the delta UAR is close to zero, it indicates the vocoder successfully preserves target speaker representations and baseline model diagnostic embeddings.")
    md_lines.append("- **Representation Drift**: Large UAR discrepancies warrant checking the specific layer representations (e.g. earlier layers vs deeper classification layer features).")
    
    with open(STAGE2_LOG_PATH, mode="w", encoding="utf-8") as f:
        f.write("\n".join(md_lines))
        
    print(f"Detailed Markdown evaluation report updated at: {STAGE2_LOG_PATH}\n")
...
def main():
    ...
    print("\n" + "#" * 60)
    print("Starting Feature Extraction for Original Pilot Set")
    print("#" * 60)
    extract_features_for_set(
        str(METADATA_DIR / "dataset_index_original_pilot.csv"),
        str(FEATURES_ORIG_DIR),
        config
    )
    
    print("\n" + "#" * 60)
    print("Starting Feature Extraction for Generated Pilot Set")
    print("#" * 60)
    extract_features_for_set(
        str(METADATA_DIR / "dataset_index_generated_pilot.csv"),
