...
"""
prepare_hifigan_full_audio.py

Purpose:
Preprocesses raw audio files for the Stage 4 full dataset experiment:
- Converts input audio to mono.
- Downsamples to 22050 Hz.
- Normalizes amplitude (peak normalization).
- Saves WAV files in the target directory (input_full_22050).
- Logs processing metadata to a CSV file in logs_full.
"""
...
INPUT_DIR = PROJECT_ROOT / "voice_conversion" / "input_full"
OUTPUT_DIR = PROJECT_ROOT / "voice_conversion" / "input_full_22050"
LOG_DIR = PROJECT_ROOT / "voice_conversion" / "logs_full"
CSV_LOG_PATH = LOG_DIR / "full_preprocessing_log.csv"
...
def preprocess_audio(file_path: Path, output_dir: Path) -> dict:
    ...
    # Ensure nested subdirectories are created if they exist
...
def main():
    print("=" * 60)
    print("Starting HiFi-GAN Full Dataset Audio Preprocessing")
    ...
    if not wav_files:
        ...
        # Create an empty CSV log with headers and a descriptive note row
        with open(CSV_LOG_PATH, mode="w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([
                ...
                "N/A", "N/A", "N/A", 
                "empty", "No WAV files are present in input_full yet"
            ])
        sys.exit(0)
